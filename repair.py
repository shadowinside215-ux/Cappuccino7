...
start_idx = code.find("                                    <span className={`px-2 py-0.5 text-[8px] font-black uppercase rounded ${order.deliveryType === 'dine-in' ? 'bg-stone-900 text-white border border-amber-500/20 shadow-lg' : 'bg-stone-100 text-stone-500'}`}>          {view === 'history' && (")

end_idx = code.find("          ) : (\n            <div className=\"flex-1 overflow-y-auto bg-bento-bg p-6 space-y-4 custom-scrollbar\">\n               <h2 className=\"text-2xl font-black uppercase italic tracking-tighter text-amber-500 mb-8\">{t('pos_unpaid_orders')}</h2>\n               \n               {/* Ready for Collection Section */}", start_idx)

if start_idx == -1 or end_idx == -1:
    print(f"Markers not found! start={start_idx} end={end_idx}")
    sys.exit(1)

replacement = """                                    <span className={`px-2 py-0.5 text-[8px] font-black uppercase rounded ${order.deliveryType === 'dine-in' ? 'bg-stone-900 text-white border border-amber-500/20 shadow-lg' : 'bg-stone-100 text-stone-500'}`}>
...
# wait, the `end_idx` includes `) : (` which is actually line 929. But what about the `pos_unpaid_orders` block? Wait, that block BELONGS! The `) : (` is NOT a closing for pending! It is part of `view === 'pending' ? ( pending... ) : ( ... )` ? NO! `view === 'pos' && ( ... )` and `view === 'pending' && ( ... )` are SEPARATE expressions!
# Oh!!!
# I see it now:
# 1. `          {view === 'pos' && (`
# 2. `          )}`
...
# 5. `          {view === 'history' && (`
# 6. `          )}`
# BUT wait! Where did the `) : (` come from?
# `) : (` at line 929 is actually the original `order.deliveryType === 'dine-in' ? ( ... ) : order.deliveryType` from the pending view! Wait, no!
# Line 930 is `<h2 className="text-2xl font-black uppercase italic tracking-tighter text-amber-500 mb-8">{t('pos_unpaid_orders')}</h2>`! But that's inside pending view!
# Why did it have `) : (`?
# Because my `fix_block.py` injected it?
# Let's just REPLACE everything from line 835 to line 950 with my clean block and let's check what's next.
# The next thing in the file is `               <div className="mb-12">`.
