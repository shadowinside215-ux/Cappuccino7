...
export default function AdminStats() {
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-bento-bg p-6 text-center text-bento-ink">
        <div className="w-20 h-20 bg-red-100 dark:bg-red-900/20 rounded-[2rem] flex items-center justify-center text-red-600 mb-6">
          <LayoutDashboard size={40} />
        </div>
        <h2 className="text-2xl font-black uppercase italic mb-2">{t('access_denied')}</h2>
        <p className="text-stone-500 max-w-xs mb-8">You don't have permission to view statistics. Please contact the administrator.</p>
        <button 
          onClick={() => navigate('/')}
          className="px-8 py-3 bg-stone-900 dark:bg-stone-800 text-white rounded-2xl font-bold text-sm"
        >
          Go Back Home
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-bento-bg p-4 sm:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-bento-card-bg p-6 rounded-[2rem] shadow-sm border border-bento-card-border flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="p-3 bg-stone-900 dark:bg-stone-700 text-white rounded-2xl">
              <TrendingUp size={20} />
            </div>
            <div className="flex items-center gap-1 text-green-500 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-lg text-[9px] font-black">
              <ArrowUpRight size={10} /> 12%
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-bento-ink tabular-nums">{stats.today.rev.toFixed(0)} MAD</p>
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mt-1">{t('today_revenue')}</p>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-bento-card-bg p-6 rounded-[2rem] shadow-sm border border-bento-card-border flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="p-3 bg-stone-900 dark:bg-stone-700 text-white rounded-2xl">
              <Calendar size={20} />
            </div>
            <div className="flex items-center gap-1 text-amber-500 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg text-[9px] font-black">
              <ArrowUpRight size={10} /> 8%
            </div>
          </div>
          <div>
            <p className="text-3xl font-black text-bento-ink tabular-nums">{stats.week.rev.toFixed(0)} MAD</p>
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest mt-1">{t('weekly_revenue')}</p>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-stone-900 dark:bg-stone-700 p-6 rounded-[2rem] shadow-sm flex flex-col justify-between text-white">
          <div className="flex justify-between items-start mb-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-bento-card-bg p-8 rounded-[2.5rem] shadow-sm border border-bento-card-border">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h3 className="text-xl font-black text-bento-ink uppercase italic tracking-tighter">Revenue Growth</h3>
              ...
              {mounted && (
                <ResponsiveContainer width="99.9%" height={320}>
                  <AreaChart data={chartData}>
                    ...
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={localStorage.getItem('theme') === 'dark' ? '#333' : '#f1f1f1'} />
                    <XAxis 
                      dataKey="name" 
                      axisLine={false} 
                      tickLine={false} 
                    ...
                    />
                    <Tooltip 
                      contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '12px', backgroundColor: localStorage.getItem('theme') === 'dark' ? '#1c1917' : '#fff', color: localStorage.getItem('theme') === 'dark' ? '#fff' : '#000' }}
                    />
                    <Area 
                      type="monotone" 
                      dataKey="revenue" 
        ...
        <div className="lg:col-span-1 bg-bento-card-bg p-8 rounded-[2.5rem] shadow-sm border border-bento-card-border">
           <h3 className="text-xl font-black text-bento-ink uppercase italic tracking-tighter mb-6">Activity Peak</h3>
           <div className="h-80 w-full min-w-0" style={{ position: 'relative', minHeight: '320px' }}>
             {mounted && (
               <ResponsiveContainer width="99.9%" height={320}>
                 <BarChart data={chartData}>
                    ...
                    <Tooltip 
                      cursor={{ fill: localStorage.getItem('theme') === 'dark' ? '#262626' : '#f8fafc' }}
                      contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '12px', backgroundColor: localStorage.getItem('theme') === 'dark' ? '#1c1917' : '#fff', color: localStorage.getItem('theme') === 'dark' ? '#fff' : '#000' }}
                    />
                    <Bar dataKey="orders" fill="#fbbf24" radius={[10, 10, 10, 10]} />
                 </BarChart>
               </ResponsiveContainer>
             )}
           </div>
      ...
      <div className="bg-bento-card-bg rounded-[2.5rem] shadow-sm border border-bento-card-border overflow-hidden">
        <div className="p-8 border-b border-bento-card-border flex justify-between items-center">
          <div>
            <h3 className="text-xl font-black text-bento-ink uppercase italic tracking-tighter">Annual Overview ({new Date().getFullYear()})</h3>
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Revenue and order distribution</p>
          </div>
          <div className="p-3 bg-bento-bg rounded-2xl">
            <History size={20} className="text-stone-500" />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {monthlyStats.map((m, i) => (
            <div key={m.month} className={`p-8 border-bento-card-border ${i % 4 !== 3 ? 'md:border-r' : ''} ${i < 8 ? 'border-b' : ''}`}>
               <p className="text-stone-400 text-[10px] font-black uppercase tracking-widest mb-1">{m.month}</p>
               <h4 className="text-2xl font-black text-bento-ink mb-4">{m.revenue.toLocaleString()} MAD</h4>
               <div className="flex items-center justify-between text-xs font-bold">
                 <span className="text-stone-400">Total Orders</span>
                 <span className="bg-bento-bg px-2 py-1 rounded-lg text-bento-ink">{m.orders}</span>
               </div>
               <div className="w-full h-1.5 bg-bento-bg rounded-full mt-4 overflow-hidden">
                  <div 
      ...
      <div className="bg-bento-card-bg rounded-[2.5rem] shadow-sm border border-bento-card-border overflow-hidden">
        <div className="p-8 border-b border-bento-card-border">
           <h3 className="text-xl font-black text-bento-ink uppercase italic tracking-tighter">Daily Ledger</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-bento-card-border text-bento-ink">
              {data.slice(0, 30).map((stat) => (
                <tr key={stat.id} className="hover:bg-bento-bg transition-colors">
                  <td className="px-8 py-6 font-black">{format(parseISO(stat.id), 'dd MMM yyyy')}</td>
                  <td className="px-8 py-6">
                    <span className="font-black">{stat.amount.toLocaleString()} MAD</span>
                  </td>
                  <td className="px-8 py-6">
                    <span className="px-3 py-1 bg-bento-bg rounded-lg font-bold text-xs">{stat.orderCount} Orders</span>
                  </td>
