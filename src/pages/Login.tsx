...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const isAdminEmail = user.email?.toLowerCase() === (import.meta.env.VITE_SUPPORT_EMAIL || 'dragonballsam86@gmail.com').toLowerCase();
      ...
      } else if (isAdminEmail && !docSnap.data().isAdmin) {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'email-signup') {
        ...
        const staffUsernames = ['admin', 'waiter', 'kitchen', 'barman', 'cashier', 'driver'];
        if (staffUsernames.includes(email.toLowerCase().trim()) || !email.includes('@')) {
          toast.error(`Please use the staff portal to log in with a staff ID.`);
          setLoading(false);
          return;
        }
