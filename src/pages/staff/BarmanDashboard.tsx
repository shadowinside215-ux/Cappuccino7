            ...
            {orders.map((order) => (
              <motion.div
                ...
                className={`bg-bento-card-bg rounded-[3rem] p-8 border ${
                  ...
                  order.barmanStatus === 'preparing' ? 'border-amber-600/30' : 'border-bento-card-border'
              ...
              >
                ...
                <div className="space-y-4 mb-8 bg-bento-bg p-6 rounded-[2rem] border border-bento-card-border text-bento-ink">
                  <p className="text-[10px] font-black text-stone-600 uppercase tracking-widest">{t('drink_orders')}</p>
                    ...
                    return (
                      <div key={idx} className="flex justify-between items-start group">
                        <div className="flex items-start gap-3">
                          <div className="mt-1">
                            {isIced ? <Snowflake size={14} className="text-blue-500" /> : isHot ? <ThermometerSun size={14} className="text-orange-600" /> : <Cup size={14} className="text-amber-700" />}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-amber-600 font-black text-lg">x{item.quantity}</span>
                ...
                <div className="flex items-center justify-between py-4 border-y border-bento-card-border mb-8">
                   <div className="flex items-center gap-2">
                     ...
                     <span className="text-[10px] font-black uppercase text-stone-400 tracking-widest text-center">EXACTLY 10 MIN</span>
                   </div>
                ...
                {order.deliveryNotes && (
                  <div className="mt-6 p-4 bg-amber-500/10 rounded-2xl border border-amber-500/20 flex items-start gap-3">
                     ...
                     <p className="text-[10px] font-bold text-amber-800/60 leading-tight italic truncate">"{order.deliveryNotes}"</p>
                  </div>
                )}
              </motion.div>
            ))}
